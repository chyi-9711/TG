...
import json
TOKEN="8101101098:AAFTKiKBQxrwS3X7bJ3mzvLjsKU6auTcKKY"
...
def send_txt(message):
    ...
    bot.send_message(message.chat.id, "5.積分點除了在最後結算有加成外，也可以拿來買線索。")
    bot.send_message(message.chat.id, "6.每小都有一次對外求助機會，積分點達到一定數額也可以兌換求助，求助內容是關於劇情推測相關的內容")
    bot.send_message(message.chat.id, "7.積分點除了在最後結算有加成外，也可以拿來買線索。")
