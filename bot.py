...
import json
TOKEN="8101101098:AAHmNHPaw9kXuJUg4VxQElxFGbwC4hjPL6I"
...
def send_txt(message):
    ...
    bot.send_message(message.chat.id, "接下來的這趟旅途我會一直陪伴大家，也會不斷給各位發布任務和提示，接下來是RPG遊戲規則介紹，要仔細閱讀喔啾咪！")
    bot.send_message(message.chat.id, "1.這次的遊戲會直接給玩家分發地圖自由活動，每個場地都可以重複過去遊玩，但要注意，我們的遊戲分成兩類：\n第一種：可以獲取線索的遊戲\n這類型的遊戲可以重複挑戰直到完成度100%，根據不同的完成度，會拿到不同的線索數量\n第二種：有積分的遊戲\n這類型的遊戲可以獲得積分(彈珠)，無法重複挑戰，積分也會進入最終結算")
    bot.send_message(message.chat.id, "2.線索一次只能選一個地方拿，例如教室和辦公室，雖然同樣在學校，但一次只能選一個地方")
    bot.send_message(message.chat.id, "3.拿到線索後可以選擇拍照傳群組，不可以拿走道具，每得到一個線索，都會有一個提示詞，只要輸入提示即可從機器人身上拿到完整劇情線索")
    bot.send_message(message.chat.id, "4.遊玩過程會不定時啟動臨時任務，完成任務可以獲得積分")
    bot.send_message(message.chat.id, "5.積分除了可以計入最終結算外，也可以拿來購買線索，若是遇到流動商城(阿潛)，可以選擇花費5積分兌換任意1條線索")
    bot.send_message(message.chat.id, "6.若是將NPC的所有遊戲100%完成，在後續搜索過程中，可以直接傳送訊息或是打電話給此NPC詢問劇情相關問題")
    bot.send_message(message.chat.id, "7.如果有需要的話，可以隨時給阿潛發訊息或打電話，在搜索過程中他會不停走動，想找到他也是有難度的喔！")
    bot.send_message(message.chat.id, "8.遊戲時間是有限制的，時間到便會強制結束，並發放劇情相關題目(表單)，真相及結局選擇會等所有小隊作答完畢後統一發放")
    bot.send_message(message.chat.id, "9.我們的最終結算包含積分、劇情選擇、問卷，問卷部分會詢問有關於劇情的問題，所以一定要還原故事真相喔！")
    bot.send_message(message.chat.id, "P.S 咳咳，豆沙包提醒大家，故事裡有很~多關於場景構建的描述不太重要，想要節省時間可以跳過喔啾咪！")
    bot.send_message(message.chat.id, "豆沙包就先在這裡祝大家玩得愉快啦！這次要遊玩的故事名叫「雙生花」，是可憐的豆沙包趕工出來的遊戲，不要嫌棄它不然我會很杯桑喔！")
...
def gen_markup2():
    ...
    markup.add(InlineKeyboardButton("我還怨恨著她",callback_data="BE"),
               ...
               InlineKeyboardButton("沒什麼好說的，她是我姊姊，一直都是。",callback_data="TE"),
               )
    return markup
